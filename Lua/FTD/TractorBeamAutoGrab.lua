...
function NameScan(I, nameString)
    ...
    for index = 0, friendlyCount - 1 do
        ...
        if trimName == nameString and friendlyInfo.Valid then
            ...
            --I:Log(string.format("friendlyInfoId is %d, index is %d",friendlyInfo.Id, index))
            targetConstructIndex = friendlyInfo.Id
            --targetConstructIndex = index
            --I:Log("Returning "..targetConstructIndex.."(NOT be -1)")
    ...
    --I:Log("Returning "..targetConstructIndex.."(should be -1)")
    return targetConstructIndex
end -- close function
...
function Update(I)
    ...
    for i = 0, tractorBeamCount, 1 do
        ...
        -- here we will make the beam tell us what the ID of its current grab is
        I:Log(string.format("beamName=%s, currentTargetIndex=%d",beamName, I:Component_GetIntLogic(7, i)))
        ...
        if beamName == "Target Set" then
            ...
            --tell this tractor beam to grab the particular construct
            I:Component_SetIntLogic_1(7, i, 0, beamTargetIndex)
            I:Log(string.format("Component type 7, blockIndex %d, propIndex 0, beamTargetIndex %d",i, beamTargetIndex))
            --then turn the beam on
            I:Component_SetBoolLogic(7,beamTargetIndex,true)
